...
function likeEventHandler(e) {
  ...
  xhr.open(
    ...
    `/like/${
      isPost
        ? `post/${currentTarget.dataset.pid}`
        : `comment/${currentTarget.dataset.cid}`
    }`,
  );
  xhr.send();
}
...
function setTimerForTab() {
  timerForTab = setTimeout(() => {
    ...
    // a cursor hasn't entered any contact in 100ms after leave
...
accountHoverTabTriggers.forEach((trigger) => {
  trigger.addEventListener('mouseenter', (e) => {
    ...
    accountHoverTabTranslate(); // relocate the tab along to the new trigger
  });
...
function friendEventHandler(e) {
  const { currentTarget } = e;
  const targetUserId = currentTarget.dataset.uid;
