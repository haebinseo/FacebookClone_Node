...
const render = {
  main: (req, res, { title, user, followings, followers, friends, posts, likes, alarms }) => {
    res.render('main', {
  ...
  messenger: (req, res, { title, user, rooms, currentRoom, currentRoomIdx, messages, alarms }) => {
  ...
  profile: {
    ...
    photo: (req, res, { title, user, targetUser, photos, alarms }) => {
      res.render('pPhoto', {
        title: title || `${targetUser.name} | Facebook`,
        user: user || req.user,
        targetUser,
